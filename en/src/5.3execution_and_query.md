# Writing and Query  
Under normal circumstances, developers will use more `Writing` and `Query`, because these are basically external read and write interfaces at the business level. So in this section we look at how in `yu`
...
- `Query` is an on-chain query. It does not modify the on-chain state, so the operation will not be synchronized to the entire network. The parameter of `Query` is one more `common.Hash` than `Writing`, this parameter is a
...
- `Inject into Tripod`：When building all our `Writing` and `Query`, we need to call `tripod.SetExecs()` to inject all `Writing` into `tripod`,
